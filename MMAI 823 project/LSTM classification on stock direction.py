...
model = regressor.fit(X, y, epochs = 20, batch_size = 100)
...
predicted_stock = pd.DataFrame(predicted_stock_price,columns=['perdict direction'])
real_stock = pd.DataFrame(real_stock_price,columns=['real direction'])
...
def iden(x,y):
    if x == y:
        ...
        return 0
final["hit"] = final.apply(iden(final['real direction'],final['perdict direction']),axis =1)
...
import pickle
with open('Apple_model_classification_return.pickle','wb') as f:
    pickle.dump(regressor,f)

with open('Apple_model_classification_return.pickle','rb') as f:
    model = pickle.load(f)
